      ...
      <div className="auth-container-premium">
        <div className="auth-card glass-panel slide-up">
          <div className="auth-header" style={{ textAlign: 'center', marginBottom: '40px' }}>
            <div className="logo-icon-large" style={{ width: '80px', height: '80px', background: 'var(--green-gradient)', borderRadius: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', boxShadow: '0 20px 40px rgba(16, 185, 129, 0.2)' }}>
            ...
            <p style={{ color: 'var(--gray-500)', fontWeight: 700, fontSize: '0.9rem', marginTop: '8px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
            ...
            <>
              <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <button type="submit" className="btn-premium btn-primary-new btn-block" disabled={loading} style={{ height: '60px', fontSize: '1.1rem' }}>
                  {loading ? 'INITIALIZING...' : 'CREATE COMMAND PROFILE'}
              ...
              <button type="button" className="btn-social-premium" onClick={() => googleLogin()}>
                ...
                <span>Synchronize Google Intel</span>
              </button>
            </>
          ) : (
            <form onSubmit={handleVerifyOtp} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
               <div className="form-group">
                  <label>6-Digit Verification Pulse</label>
                  <input 
                    type="text" 
                    style={{ textAlign: 'center', fontSize: '2.5rem', letterSpacing: '12px', fontWeight: 900, height: '80px', background: 'rgba(255,255,255,0.02)', borderRadius: '24px' }}
                    placeholder="000000" 
                    value={otp} 
                    maxLength={6}
               ...
               <button type="submit" className="btn-premium btn-primary-new btn-block" style={{ height: '60px' }} disabled={loading}>
                  {loading ? 'VERIFYING...' : 'CONFIRM IDENTITY'}
                  <ShieldCheck size={20} />
               </button>

               <div style={{ textAlign: 'center' }}>
                  <p style={{ color: 'var(--gray-500)', fontSize: '0.9rem', marginBottom: '8px' }}>Signal not received?</p>
                  <button type="button" style={{ background: 'none', border: 'none', color: 'var(--blue-light)', fontWeight: 800, cursor: 'pointer' }} onClick={async () => {
          ...
          <div className="auth-footer" style={{ marginTop: '40px', textAlign: 'center', fontSize: '0.9rem', color: 'var(--gray-500)' }}>
            Already Enlisted? <Link to="/login" style={{ color: 'var(--blue-light)', fontWeight: 800, textDecoration: 'none' }}>Access Terminal</Link>
          </div>
        </div>
      </div>
